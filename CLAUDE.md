...
فاز ۰، ۱، ۲ و ۳ تکمیل شده‌اند، شامل لوگوی واقعی و عکس واقعی هدر. سایت روی Cloudflare Workers زنده است: https://devino.amirbagmaz2003.workers.dev — افکت زوم هدر (روی عکس واقعی)، گذر رنگ اسکرول، هدر/فوتر fixed، و لوگوی واقعی با کراس‌فید پیاده‌سازی شده‌اند (دستورالعمل در `phase-3-header-brief.md`).

**عکس هدر:** یکی از ده عکس مُد ارسالی («silhouette-bw» با نام فایل `hero-editorial-bw.jpg`) به‌عنوان عکس هدر انتخاب شد — تنها عکس کاملاً سیاه‌وسفید بین آن‌ها، پس با پالت رسمی سایت (مشکی/سفید صدفی) بدون هیچ تناقض رنگی هم‌خوانی کامل دارد و لحن «دراماتیک، مینیمال» برند را هم منتقل می‌کند. نه عکس دیگر (لباس‌های رنگی، عکس ساکن لیوان شراب و...) در `public/photos/` ذخیره شده‌اند، برای استفاده‌ی احتمالی در فازهای بعدی (گرید کالکشن‌ها در فاز ۴، صفحه‌ی درباره‌ی ما، و غیره) — هنوز جایی در کد استفاده نشده‌اند.
