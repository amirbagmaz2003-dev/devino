...
export default function OrderForm({
  ...
  const [state, dispatch, pending] = useActionState<OrderFormState, FormData>(
    submitOrderAction,
    {
      status: "idle",
    },
  ...
  useEffect(() => {
    ...
    if (first)
      formRef.current?.querySelector<HTMLElement>(`#order-${first}`)?.focus();
  }, [state]);
  ...
  const describe = (field: OrderFieldError, extra?: string) =>
    [errors.has(field) ? `order-${field}-error` : null, extra]
      .filter(Boolean)
      .join(" ") || undefined;
  const invalid = (field: OrderFieldError) => errors.has(field) || undefined;
  const error = (field: OrderFieldError) =>
    errors.has(field) ? t(`errors.${field}`) : null;

  // After a successful order only the confirmation remains: no heading,
  // intro or product summary — focus moves to it for screen readers.
  if (state.status === "success") {
    return (
      <div className="py-12 text-center">
        <p
          ...
          tabIndex={-1}
          role="status"
          className="font-heading mx-auto max-w-xl text-xl leading-relaxed focus:outline-none sm:text-2xl"
        >
          {t("success", { method: t(`successMethods.${state.method}`) })}
        </p>
        <Link
          href="/collections"
          className="text-matte-black/70 hover:text-olive-accent mt-8 inline-flex min-h-11 items-center text-sm underline underline-offset-4"
        >
          {t("backToCollections")}
        </Link>
      </div>
    ...
    <>
      {product && (
        <div
          ...
          className="border-matte-black/15 mb-10 flex items-center gap-5 border-b pb-6"
        >
          <div className="bg-matte-black/5 relative aspect-[3/4] w-20 shrink-0 overflow-hidden">
            {product.image && (
              <MediaBox {...product.image} zoom={false} sizes="80px" />
            )}
          </div>
          <div>
      ...
      >
        ...
        <fieldset>
          <legend className="mb-3 block text-sm">
